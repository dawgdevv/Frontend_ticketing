        ...
        <div className="max-w-4xl mx-auto px-4 py-8">
            ...
            <div className="bg-primary/10 p-6 rounded-lg shadow-lg">
                ...
                <ul className="space-y-6">
                    {events.map((event) => (
                        <li key={event._id} className="bg-background p-4 rounded-md shadow transition-all hover:shadow-md">
                            <h3 className="text-xl font-semibold mb-2 text-primary">{event.name}</h3>
                            <p className="text-muted-foreground">
                                <span className="font-medium">Date:</span> {new Date(event.date).toLocaleString('en-US', { year: 'numeric', month: 'long', day: 'numeric', hour: 'numeric', minute: 'numeric' })}
